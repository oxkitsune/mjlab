import gymnasium as gym

gym.register(
  id="Mjlab-Velocity-Rough-Booster-K1",
  entry_point="mjlab.envs:ManagerBasedRlEnv",
  disable_env_checker=True,
  kwargs={
    "env_cfg_entry_point": f"{__name__}.rough_env_cfg:BoosterK1RoughEnvCfg",
    "rl_cfg_entry_point": f"{__name__}.rl_cfg:BoosterK1PPORunnerCfg",
  },
)

gym.register(
  id="Mjlab-Velocity-Rough-Booster-K1-Play",
  entry_point="mjlab.envs:ManagerBasedRlEnv",
  disable_env_checker=True,
  kwargs={
    "env_cfg_entry_point": f"{__name__}.rough_env_cfg:BoosterK1RoughEnvCfg_PLAY",
    "rl_cfg_entry_point": f"{__name__}.rl_cfg:BoosterK1PPORunnerCfg",
  },
)

gym.register(
  id="Mjlab-Velocity-Flat-Booster-K1",
  entry_point="mjlab.envs:ManagerBasedRlEnv",
  disable_env_checker=True,
  kwargs={
    "env_cfg_entry_point": f"{__name__}.flat_env_cfg:BoosterK1FlatEnvCfg",
    "rl_cfg_entry_point": f"{__name__}.rl_cfg:BoosterK1PPORunnerCfg",
  },
)

gym.register(
  id="Mjlab-Velocity-Flat-Booster-K1-Play",
  entry_point="mjlab.envs:ManagerBasedRlEnv",
  disable_env_checker=True,
  kwargs={
    "env_cfg_entry_point": f"{__name__}.flat_env_cfg:BoosterK1FlatEnvCfg_PLAY",
    "rl_cfg_entry_point": f"{__name__}.rl_cfg:BoosterK1PPORunnerCfg",
  },
)
