...
class UniformCrawlVelocityCommand(CommandTerm):
  cfg: UniformVelocityCommandCfg
  ...
  def __init__(self, cfg: UniformVelocityCommandCfg, env: ManagerBasedRlEnv):
    ...
    # vel_command_b layout:
    #   0: commanded v_y (body y, in locomotion plane)
    #   1: commanded v_z (body z, in locomotion plane)
    #   2: commanded ω_x (yaw around body x)
    self.vel_command_b = torch.zeros(self.num_envs, 3, device=self.device)
    self.heading_target = torch.zeros(self.num_envs, device=self.device)
    self.heading_error = torch.zeros(self.num_envs, device=self.device)
    self.is_heading_env = torch.zeros(
    ...
    # Name kept for compatibility, but this is (y, z) error now.
    self.metrics["error_vel_xy"] = torch.zeros(self.num_envs, device=self.device)
    self.metrics["error_vel_yaw"] = torch.zeros(self.num_envs, device=self.device)
  ...
  def _update_metrics(self) -> None:
    ...
    # Planar linear velocity error in (y, z).
    vel_cmd_yz = torch.stack(
      [self.vel_command_b[:, 0], self.vel_command_b[:, 1]], dim=-1
    )
    vel_act_yz = torch.stack(
      [
        self.robot.data.root_link_lin_vel_b[:, 1],  # v_y
        self.robot.data.root_link_lin_vel_b[:, 2],  # v_z
      ],
      dim=-1,
    )
    self.metrics["error_vel_xy"] += (
      torch.norm(vel_cmd_yz - vel_act_yz, dim=-1) / max_command_step
    )

    # Yaw / heading now uses ω_x (body x).
    self.metrics["error_vel_yaw"] += (
      torch.abs(self.vel_command_b[:, 2] - self.robot.data.root_link_ang_vel_b[:, 0])
      / max_command_step
    )
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    # Sample planar linear velocities in (y, z):
    #   - lin_vel_x: forward (body z)
    #   - lin_vel_y: lateral (body y)
    ...
    # Angular: yaw around body x, magnitude from ang_vel_z range (reuse).
    self.vel_command_b[env_ids, 2] = r.uniform_(*self.cfg.ranges.ang_vel_z)  # ω_x
    ...
    # Optionally initialize the root state with the commanded velocities.
    init_vel_mask = r.uniform_(0.0, 1.0) < self.cfg.init_velocity_prob
    init_vel_env_ids = env_ids[init_vel_mask]
    if len(init_vel_env_ids) > 0:
      root_pos = self.robot.data.root_link_pos_w[init_vel_env_ids]
      ...
      lin_vel_b = self.robot.data.root_link_lin_vel_b[init_vel_env_ids]
      # Map command → body-frame velocities: v_y (lateral), v_z (forward).
      lin_vel_b[:, 1] = self.vel_command_b[init_vel_env_ids, 0]
      lin_vel_b[:, 2] = self.vel_command_b[init_vel_env_ids, 1]
      root_lin_vel_w = quat_apply(root_quat, lin_vel_b)

      root_ang_vel_b = self.robot.data.root_link_ang_vel_b[init_vel_env_ids]
      # Yaw around body x.
      root_ang_vel_b[:, 0] = self.vel_command_b[init_vel_env_ids, 2]
  ...
  def _update_command(self) -> None:
    if self.cfg.heading_command:
      # heading_w still assumed to be "yaw" in world frame provided by the sim.
      self.heading_error = wrap_to_pi(self.heading_target - self.robot.data.heading_w)
      env_ids = self.is_heading_env.nonzero(as_tuple=False).flatten()
      self.vel_command_b[env_ids, 2] = torch.clip(
        self.cfg.heading_control_stiffness * self.heading_error[env_ids],
  ...
  def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
    """Draw velocity command and actual velocity arrows for (y, z) locomotion."""
    ...
    # Command linear velocity arrow (blue) in (y, z).
    cmd_lin_from = local_to_world(np.array([0.0, 0.0, z_offset]) * scale)
    cmd_lin_to = local_to_world(
      (np.array([0.0, 0.0, z_offset]) + np.array([0.0, cmd[0], cmd[1]])) * scale
    )
    visualizer.add_arrow(
      cmd_lin_from, cmd_lin_to, color=(0.2, 0.2, 0.6, 0.6), width=0.015
    )

    # Command angular velocity arrow (green): yaw around body x.
    cmd_ang_from = cmd_lin_from
    cmd_ang_to = local_to_world(
      (np.array([0.0, 0.0, z_offset]) + np.array([cmd[2], 0.0, 0.0])) * scale
    )
    visualizer.add_arrow(
      cmd_ang_from, cmd_ang_to, color=(0.2, 0.6, 0.2, 0.6), width=0.015
    )

    # Actual linear velocity arrow (cyan) in (y, z).
    act_lin_from = local_to_world(np.array([0.0, 0.0, z_offset]) * scale)
    act_lin_to = local_to_world(
      (np.array([0.0, 0.0, z_offset]) + np.array([0.0, lin_vel_b[1], lin_vel_b[2]]))
      * scale
    )
    visualizer.add_arrow(
      act_lin_from, act_lin_to, color=(0.0, 0.6, 1.0, 0.7), width=0.015
    ...
    # Actual angular velocity arrow (light green): yaw around body x.
    act_ang_from = act_lin_from
    act_ang_to = local_to_world(
      (np.array([0.0, 0.0, z_offset]) + np.array([ang_vel_b[0], 0.0, 0.0])) * scale
    )
    visualizer.add_arrow(
      act_ang_from, act_ang_to, color=(0.0, 1.0, 0.4, 0.7), width=0.015
    )
